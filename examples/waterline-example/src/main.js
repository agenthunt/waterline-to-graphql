...
function setupAssociations(models) {
  _.each(models, function eachInstantiatedModel(thisModel, modelID) {
    ...
    thisModel.associations = _.reduce(
      ...
      function(associatedWith, attrDef, attrName) {
        if (
...
waterlineInstance.initialize(config, (err, initializedWaterlineInstance) => {
  ...
  initializeModelsWithData(models)
    .then(function(intialized) {
      ...
      let mutations =
        'mutation { createUser(firstName:"hello", lastName:"Hellosson", email: "hello@hellosson.com"){ firstName, lastName}}';
